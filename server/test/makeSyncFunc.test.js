...
test("makeSyncFunc 将异步代码转为同步代码", () => {
  ...
  }
  {
    ...
    var t = makeSyncFunc(a1.toString()).toString().replace(/\s+/g,"");
     //toString会把没有用的()去掉，而！这样会添加（）
     expect(t).toBe('function({parama:{},paramb}){letb={};leta=1+2;lettest=function(){letd=1;};lett=testSync();if(a){b=1;}if(!(testSync())){b=3;}letobj={test:test};letg=\'1async\\\'\';letd=obj["test"+"Sync"]();}');
   }
