...
//解析参数
const convertParams = require("enhanced-resolve/lib/convertParams");
//处理路径
function join(a, b) {
  return toPath(a + "/" + b)
}
...
class CachedInputFileSystem {
  constructor(duration, nocache) {
    ...
    ["getSize", "getModify", "isDir", "isFile", "exists"].forEach(type => {
      let handleCallback = data => {
        switch (type) {
          case "getSize":
            return data.size;
          case "getModify": //精确到10ms
            return Math.floor(data.mtimeMs);
          case "isDir":
    ...
    ["findFile", "findDir", "findAll"].forEach(api => {
      ...
      if (!this[api]) {
        this[api] = (path, fileType, deep, filterFn, callback) => {

          var params;
          if (typeof path == "object") {
            params = path;
          } else {
            params = convertParams(path, fileType, deep, filterFn, callback);
          }
