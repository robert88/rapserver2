  ...
  static fileSystemBindCache(type, duration, syncFlag,nocache) {
    ...
    if (fileSystem[api] && !this[api]) {
      this[api] = (path, success, error) => {
        //异步返回是空，同步callback无效
        let  handleApiCallback =  function(err, data) {
          if (err) {
        ...
        try {
          //不需要使用缓存的，和需要使用缓存的
          if(nocache){
            return fileSystem[api](path,handleApiCallback);
          }else{
            return this[storageId]["provide" + syncFlag](path, fileSystem[api].bind(fileSystem),handleApiCallback);
          }
        } catch (err) {
