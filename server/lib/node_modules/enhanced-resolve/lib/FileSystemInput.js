  ...
  static filesystemBind(type, duration, nocache) {
    ...
    if (!nocache) {
      ...
      this[api] = async (path, callback) => {
        let ret = await this[storageId].provide(path,async (path)=>{
          return await fileSystem[api](path)
        });
        if (typeof callback == "function") { callback(ret); }
        return ret;
      }
    ...
    //创建同步代码,需要重新建立闭包
    this[api + "Sync"] =new Function("storageId","api","fileSystem","return "+makeSyncFunc(this[api]).toString())(storageId,api,fileSystem) ;
