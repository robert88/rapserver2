...
class FileSystemInput {
  ...
  static filesystemBind(type, duration, nocache) {
    ...
    let promise_callback;
    let promise_callbackSync;
    ...
    if (!nocache) {
      ...
      this[api] = async (path, callback) => {
        let ret = await new Promise((resolve, reject) => {
          this[storageId].provide(path, fileSystem[api].bind(fileSystem), (err, result) => {
            if (err) {
              reject(err);
            } else {
              resolve(result);
      ...
      this[api+"Sync"]= (path, callback) => {
        let ret = this[storageId].provideSync(path, fileSystem[api].bind(fileSystem));
        if (typeof callback == "function") { callback(ret); }
        return ret;
      }
      ...
      this[api] = async (path, callback) => {
        let ret = await new Promise((resolve, reject) => {
          fileSystem[api](path, (err, result) => {
            if (err) {
              reject(err);
            } else {
              resolve(result);
      ...
      this[api+"Sync"]= (path, callback) => {
        let ret =  fileSystem[api+"Sync"](path);
        if (typeof callback == "function") { callback(ret); }
        return ret;
      }
    }
  ...
  static extendApi(type, invokeType, handleCallback) {
    ...
    if (this[invokeApi] && !this[api]) {
      ...
      //创建同步代码
      this[api + "Sync"] = new Function("invokeApi", "handleCallback", "return " + makeSyncFunc(this[api]).toString())(invokeApi, handleCallback);
    }
