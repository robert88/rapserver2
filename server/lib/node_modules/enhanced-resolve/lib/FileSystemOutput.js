...
class ConverOutPut {
  constructor() {
    ["mkdir", "rename", "appendFile", "writeFile", "unlink", "rmdir", "copyFile", "open", "write", "fsync", "close"].forEach(api => {
      this[api] = fs[api];
...
class FileSystemOutput {
  ...
  // writeSplit,续点写入数据
  async writeSplit(file, buffer, position) {
    ...
    // 打开文件
    let fd = await this.output.open(file, "r+");
    // fd：文件描述符，需要先使用 open 打开；
    // buffer：存储将要写入文件数据的 Buffer；
    // offset：整数，从 Buffer 读取数据的初始位置；
    // length：整数，读取 Buffer 数据的字节数；
    // position：整数，写入文件初始位置；
    // 读取 buf 向文件写入数据offset：整数；
    await this.output.write(fd, buffer, 0, buffer.byteLength, position);

    // 同步磁盘缓存
    await this.output.fsync(fd);

    // 关闭文件
    await this.output.close(fd);
