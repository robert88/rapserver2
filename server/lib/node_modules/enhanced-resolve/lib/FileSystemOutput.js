...
class ConverOutPut{
  constructor(need) {
    ["mkdir","rename","appendFile","writeFile","unlink","rmdir","createReadStream","createWriteStream"].forEach(api=>{
      if(need){
        this[api]=makeCallbackToPromise(fs[api]);
      }else{
        this[api]= fs[api];
      }
    })
...
class FileSystemOutput {
  ...
  constructor(system,debug) {
    if(debug){
      this.system = system || Object.assign(new ConverOutPut(true),makeCallbackToPromise(new FileSystemInput(5000),["wake"]));
    }else{
      this.system = system || Object.assign(new ConverOutPut(),new FileSystemInput(5000));
    }
  ...
  async create(createDirPath, callback) {
    ...
    if (parentDirPath && !await this.system.exists(parentDirPath)) {
      await this.create(parentDirPath);
    }
