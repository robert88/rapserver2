...
module.exports = class CachedInputFileSystem {
	constructor(fileSystem, duration) {
		...
		["readJson","readData"].forEach(type=>{
			CachedInputFileSystem.extendApi.call(this,type,"readFile",buffer=>{
				if(type=="readJson"){
	...
	static fileSystemBindCache(type,duration){
		...
		CachedInputFileSystem.exportApi.call(this,type,api);
	}
	...
	//输出接口
	static exportApi(type,api){

		//添加异步api
		if(	this["_"+type] ){
			this[type] = (path,callback)=>{
				this["_"+api+"Storage"].provide(path,this["_"+type], callback);
			}
		} 
		...
		if(["_"+type+"Sync"] ){
			this[type+"Sync"] = (path)=>{
				return this["_"+api+"Storage"].provideSync(path,this["_"+type+"Sync"]);
			}
		} 
