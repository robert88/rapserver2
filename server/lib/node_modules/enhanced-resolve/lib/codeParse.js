...
const generate = require("esprima/generate-unicode-regex");
...
function generateCode(code) {
  ...
  let Token = [];
  let matchdKey;
  for (let i = 0; i < len; i++) { //这个性能要优于while
    let key = code[i];
    let pervKey = code[i - 1];
    let unicodeKey = key.charCodeAt();

    if (matchdKey = filterByClose(["'", '"', '`', '/'], key, pervKey, matchdKey)) {
      splitCode += key;
      continue;
    }
    if (generate.isIdentifierStart(unicodeKey)) {
      splitCode += key;
      continue;
    } else {
      if (splitCode) {
        Token.push(splitCode);
        splitCode = "";
      }
      let mergeNextKey = key + code[i + 1] + code[i + 2] + code[i + 3];
      var match = /^([\|&><]{2,4})/.exec(mergeNextKey) || /(^[!=><\+\-\*%]={1,3})/.exec(mergeNextKey) || /^(=>)/.exec(mergeNextKey);
      if (match) {
        i += match[0].length - 1;
        Token.push(match[0]);
        continue;
      }
      if (!generate.isWhiteSpace(unicodeKey)) {
        //两个行变一行
        if (!(generate.isLineTerminator(unicodeKey) && (generate.isLineTerminator(Token[Token.length - 1].charCodeAt())))) {
          Token.push(key);
        }
      }
    }
